...
HospTab.prototype.onSelectionChangeHistogram = function (s) {
    ...
    this.data_fil = that.geodata.filter(function (d) {
    ...
    this.list_country_hosp = this.fil_data.map(function (d) {
        return d.properties.name;
    });

    this.hdata_t = this.hdata.filter(function (d) {
        return that.list_country_hosp.indexOf(d.key) > -1;
    });
